...
| UNIT-04 | Unit | `api-spec.md` §4, BR-10 | Ticket-list query normalization given invalid `sort`, `order`, `page`, `pageSize`, and given valid ones | Each invalid value replaced by its documented default and each valid value passed through, with no error thrown | `server/tests/lab-02/ticket-list-query.unit.test.ts` | Planned |

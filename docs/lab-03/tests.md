...
| A11Y-01 | Accessibility | `ui-spec.md` §11 | Keyboard only, no mouse: Login, Change Password, Queue search/filter/sort/pagination, Staff Ticket Detail owner/IT Priority/status controls and comment/note composers, User Management list and panel | Every control reachable with Tab, operable with Enter/Space/arrow keys, with a visible focus indicator | `e2e/lab-03/accessibility.spec.ts` | Pass |

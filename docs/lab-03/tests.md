...
| API-44 | API | FR-11 | `GET /api/staff/tickets/:id` as IT Staff, for a missing id, and as a Requester | Full Ticket with requester, owner and attachments and no comment or note fields; `404` for a missing id; `403` for a Requester | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |

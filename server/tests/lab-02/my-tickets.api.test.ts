...
const MARK = 'ZQ7X';
const EMAIL_PREFIX = 'lab2-my-tickets-test-';
...
describe('GET /api/tickets', () => {
  ...
  const listAs = (cookie: string, query = '') => request(app).get(`/api/tickets${query}`).set('Cookie', cookie);
  ...
  beforeAll(async () => {
    ...
    const emptyRequester = await makeRequesterFixture(`${EMAIL_PREFIX}empty@toktickit.dev`);

    requesterACookie = await sessionCookieFor(requesterA.id);
    requesterBCookie = await sessionCookieFor(requesterB.id);
    emptyRequesterCookie = await sessionCookieFor(emptyRequester.id);
  ...
  it('returns only the calling Requester’s own Tickets', async () => {
    const asA = await listAs(requesterACookie, `?search=${MARK}`);
    const asB = await listAs(requesterBCookie, `?search=${MARK}`);
  ...
  it('rejects the request with 401 when there is no session (BR-08)', async () => {
  ...
  it('returns the documented list-item shape, with Category as a named object', async () => {
    const response = await listAs(requesterACookie, `?search=${MARK} Printer`);
  ...
  it('searches Summary and Ticket Number, case-insensitively and partially (BR-09)', async () => {
    const bySummary = await listAs(requesterACookie, `?search=${MARK} Printer`);
    const byLowercase = await listAs(requesterACookie, `?search=${MARK.toLowerCase()} printer`);
    const byTicketNumber = await listAs(requesterACookie, '?search=2099-900003');
  ...
  it('filters by Category, and combines the filter with the search term', async () => {
    const response = await listAs(requesterACookie, `?search=${MARK}&category=${categoryOneId}`);
  ...
  it('filters by Requested Priority', async () => {
    const response = await listAs(requesterACookie, `?search=${MARK}&requestedPriority=HIGH`);
  ...
  it('defaults to Ticket Date descending and sorts ascending on request (BR-10)', async () => {
    const byDefault = await listAs(requesterACookie, `?search=${MARK}`);
    const ascending = await listAs(requesterACookie, `?search=${MARK}&sort=ticketDate&order=asc`);
  ...
  it('sorts by Ticket Number and by Requested Priority', async () => {
    const byNumber = await listAs(requesterACookie, `?search=${MARK}&sort=ticketNumber&order=asc`);
    const byPriority = await listAs(requesterACookie, `?search=${MARK}&sort=requestedPriority&order=desc`);
  ...
  it('paginates with stable, non-overlapping pages', async () => {
    const pageOne = await listAs(requesterACookie, `?search=${MARK}&pageSize=5&page=1`);
    const pageTwo = await listAs(requesterACookie, `?search=${MARK}&pageSize=5&page=2`);
  ...
  it('returns an empty page rather than an error past the last page', async () => {
    const response = await listAs(requesterACookie, `?search=${MARK}&pageSize=5&page=9`);
  ...
  it('treats a malformed filter as matching nothing, never as absent', async () => {
    const badStatus = await listAs(requesterACookie, `?search=${MARK}&currentStatus=CLOSED`);
    const badCategory = await listAs(requesterACookie, `?search=${MARK}&category=abc`);
  ...
  it('returns an empty page for a Requester who owns no Tickets (AC-16)', async () => {
    const response = await listAs(emptyRequesterCookie);
  ...
  it('returns an empty page for a search that matches nothing (AC-17)', async () => {
    const response = await listAs(requesterACookie, '?search=zzzz-no-such-ticket-anywhere');
  ...
  it('sorts by updatedAt, putting the most recently changed Ticket first', async () => {
    ...
    const response = await listAs(requesterACookie, `?search=${MARK}&sort=updatedAt&order=desc`);

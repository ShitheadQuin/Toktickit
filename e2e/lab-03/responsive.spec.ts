...
//
// The file is in two parts on purpose. User Management lists every User row in the database, so its
// captures run last, after the authentication and staff fixtures have been torn down - otherwise
// the graded evidence shows a list of Playwright fixture accounts rather than the seeded ones.
...
test.describe('RESP-01 — authentication, Queue and Ticket Detail', () => {
...
// Declared second, and with workers: 1 in playwright.config.ts the file runs in order, so by the
// time these execute the describe above has torn its fixture accounts down.
test.describe('RESP-01 — User Management', () => {
  test.beforeAll(async () => {
